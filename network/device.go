...
type IPDevice interface {
	// Close closes this device. Any future [Read] or [Write] operations will return [ErrClosed] error.
	Close() error
	...
	//
	// If len(`p`) is smaller than the incoming IP packet, only len(`p`) bytes will be copied to `p`, the excess bytes
	// are discarded (this aligns with the socket recvfrom function), and nil error will be returned. You can Use [MTU]
	// to get the maximum size of the packets.
	//
