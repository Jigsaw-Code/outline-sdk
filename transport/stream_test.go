...
// Make sure there are no connection leakage in WrapConnFuncStreamDialer
func TestWrapConnFuncStreamDialerCloseInnerConnOnError(t *testing.T) {
	errWrapConnError := errors.New("error thrown in WrapConn")
	inner := &connsCounterStreamDialer{}
	d := &WrapConnFuncStreamDialer{
		Dialer: inner,
		WrapConn: func(StreamConn) (StreamConn, error) {
			return nil, errWrapConnError
		},
	}
	conn, err := d.DialStream(context.Background(), "127.0.0.1:8888")
	require.Error(t, err)
	require.ErrorIs(t, err, errWrapConnError)
	require.Nil(t, conn)
	require.Zero(t, inner.activeConns)
}

// connsCounterStreamDialer is a StreamDialer that counts the number of active StreamConns.
type connsCounterStreamDialer struct {
	activeConns int
}

func (d *connsCounterStreamDialer) DialStream(ctx context.Context, raddr string) (StreamConn, error) {
	d.activeConns++
	return &connsCounterStreamConn{d: d}, nil
}

type connsCounterStreamConn struct {
	StreamConn
	d *connsCounterStreamDialer
}

func (c *connsCounterStreamConn) Close() error {
	c.d.activeConns--
	return nil
}
