...
The [`resolve` tool](https://pkg.go.dev/github.com/Jigsaw-Code/outline-sdk/tools/resolve) resolves a domain name, similar to `dig`:

```console
$ go run github.com/Jigsaw-Code/outline-sdk/tools/resolve@latest -type A -transport "tls" -resolver 8.8.8.8:853 -tcp getoutline.org.
216.239.34.21
216.239.32.21
216.239.38.21
216.239.36.21
...
The [`fetch` tool](https://pkg.go.dev/github.com/Jigsaw-Code/outline-sdk/tools/fetch) fetches
a URL, similar to `curl`. The example below would bypass blocking of `meduza.io` in Russia:

```console
$ go run github.com/Jigsaw-Code/outline-sdk/tools/fetch@latest -transport "override:host=cloudflare.net|tlsfrag:1" -method HEAD -v https://meduza.io/
...
The [`http2transport` tool](https://pkg.go.dev/github.com/Jigsaw-Code/outline-sdk/tools/http2transport) runs a local proxy that creates connections according to the transport. It's effectively a circumvention tool.
...
```console
$ go run github.com/Jigsaw-Code/outline-sdk/tools/http2transport@latest -transport "override:host=cloudflare.net|tlsfrag:1" -localAddr localhost:8080
...
The [`test-connectivity` tool](https://pkg.go.dev/github.com/Jigsaw-Code/outline-sdk/tools/test-connectivity) is useful to test connectivity to a proxy. It uses DNS resolutions over TCP and UDP using the transport to test if there is stream and datagram connectivity.

```console
$ go run github.com/Jigsaw-Code/outline-sdk/tools/test-connectivity@latest -transport "$OUTLINE_KEY" && echo success || echo failure
...
The [`fetch-speed` tool](https://pkg.go.dev/github.com/Jigsaw-Code/outline-sdk/tools/fetch-speed) fetches
